...
export default {
  ...
  actions: {
    ...
    async getUser({ commit, dispatch, getters }, userId) {
      ...
      const test = await baseAxios.get(`/users/${userId}.json`)
      console.log(test, userId)
      commit('setUser', {...data, id: userId})
      commit('clearMessage', null, {root: true})
    },
    async test({ commit, dispatch, getters }, payload) {
      const {data} = await baseAxios.post(`/users/${payload.userId}.json`, payload.user)
    },
